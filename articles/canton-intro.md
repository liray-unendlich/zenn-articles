...
なお、 Canton Coin が存在するのは、デフォルトで存在する公開された同期ドメインである Global Synchronizer の運用するネットワークです。下図は、Global Synchronizer と各 Sync Domain の関係、およびドメインを跨ぐトランザクションの処理フローを示すものです。
     ...
     ▼                              ▼                               ▼
┌─────────────────┐           ┌───────────────┐           ┌───────────────┐
│ 非公開Domain A  │           │ 非公開Domain B │           │ 非公開同期ドメイン C │
│ (金融機関)      │           │ (カストディアン) │           │ (その他)        │
│                │           │                │           │                │
└────────────────┘           └────────────────┘           └────────────────┘
      │                              │                               │
...
ここまで、 Canton Network が金融機関の求める要求事項を満たし、分離されたネットワークを持つ分散型台帳であることを示しました。また R3 Corda に非常に似通った仕様を持っているものの、相互運用性を重要視し、かかわるユースケースに力を注いでいることもわかっていただけたと思います。
