---
title: "世界の金融・テック巨人が名を連ねるCanton Networkの概要と強み"
emoji: "🏦"
type: "tech" # tech: 技術記事 / idea: アイデア
topics: ["Canton Network", "暗号資産", "RWA", "DeFi", "ブロックチェーン"]
published: true
publication_name: omakase
published_at: 2025-12-18 15:00
---

## 本記事の概要

本記事では、ゴールドマン・サックス、JPモルガン、BNPパリバ、マイクロソフト、Deloitte等世界の金融・テック巨人が名を連ねる分散型「金融機関向けネットワーク」、Canton Networkの概要と強みを説明します。
...
金融機関では、利用するシステムに対し以下に示す要件を求めます。この要件の多くは、PFMI（金融市場インフラの国際基準）や金融庁のガイドラインにより要請されるものです。
...
1. ファイナリティ
     ...
     PFMI においては、"final settlement is final" 決済確定後に巻き戻することがないよう求められており、確率的ファイナリティでは認められない場合が生じます。多くのチェーンにおいては解決しつつあります。

こういった複数の制約により、一概にパブリックブロックチェーンで金融市場インフラを構築できるとは言えません。金融インフラ水準としては、制御可能なプライバシー機能、ファイナリティ、ガバナンスや責任分離が求められます。
...
Canton Coin が存在するのは、デフォルトで存在する公開された同期ドメイン、 Global Synchronizer の運用するネットワークです。下図は、Global Synchronizer と各 Sync Domain の関係及びドメインを跨ぐトランザクションの処理フローを示すものです。
...
```mermaid
flowchart TD
    EthMainnet["Ethereum Mainnet<br>（**単一のグローバル台帳**）<br>- Validator(バリデータ)<br>- Block Proposer(提案者)<br>- MEV Searchers(アービトラージ等)<br>- MEV Builders(ブロック構築)<br>- MEV Relayers(ブロック中継)"]
...
続いて、 Canton Network のアーキテクチャを確認しましょう。
<!-- textlint-disable -->
